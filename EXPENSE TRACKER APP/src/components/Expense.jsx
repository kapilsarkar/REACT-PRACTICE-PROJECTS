...
const Expense = () => {
  ...
  return (
    <div className="w-[77rem] mx-auto h-auto bg-white text-black dark:bg-gray-900 dark:text-white border border-gray-300  rounded-md p-5 mt-2 shadow-lg">
      ...
      {expenses.length === 0 ? (
        <p className="text-center text-red-500 dark:text-white">No expenses yet.</p>
      ) : (
        <ul className=" space-y-2 flex flex-wrap gap-2 shadow-2xl">
          {expenses.map((exp) => (
            <li key={exp.id} className="border bg-blue-500 text-white font-bold mt-2 p-2  rounded-md shadow-md">
